...
class ImageHelper:
        ...
        with self._resolve_lock:
            ...
            if self.glob:
                ...
                if globals.settings.tex_compress is TexCompress.ASTC:
                    # Prefer ASTC, then WebM animations, then GIFs.
                    sorting = lambda path: 1 if path.name.endswith(".astc.ktx.zst") else 2 if path.suffix == ".webm" else 3 if path.suffix == ".gif" else 4 if not path.name.endswith(".bc7.ktx.zst") else 5
                elif globals.settings.tex_compress is TexCompress.BC7:
                    # Prefer BC7, then WebM animations, then GIFs, then anything else.
                    sorting = lambda path: 1 if path.name.endswith(".bc7.ktx.zst") else 2 if path.suffix == ".webm" else 3 if path.suffix == ".gif" else 4 if not path.name.endswith(".astc.ktx.zst") else 5
                else:
                    # Prefer WebM animations, then GIFs, and avoid compressed files.
                    sorting = lambda path: 1 if path.suffix == ".webm" else 2 if path.suffix == ".gif" else 3 if path.suffix != ".zst" else 4
    ...
    def _apply(self, apply_time_max: float):
        if not self.textures:
            # Loading was cancelled before GPU upload.
            if self.texture_ids:
    ...
    def texture_id(self):
        ...
        if not self.loaded:
            if not self.loading:
                ...
                sync_thread.queue(self._load)  # changed
        else:
